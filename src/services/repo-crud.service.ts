...
export class RepoCRUDService<T extends BaseEntity> extends RepoReadService<T> {
  ...
  async entityUpdate(e: T, dto: IBaseEntityUpdateDTO, map: SimpleMap = {}): Promise<T> {
    this.logger.debug(`Try to update ${JSON.stringify(this.entityName)} #${e.id} with params #${JSON.stringify(dto)} and map ${JSON.stringify(map)}...`);
    e.updateFromDTO(dto);
  ...
  async entityDelete(e: T, map: SimpleMap = {}): Promise<T> {
    this.logger.debug(`Try to delete ${JSON.stringify(this.entityName)} #${e.id} with map ${JSON.stringify(map)}...`);
    e.delete();
